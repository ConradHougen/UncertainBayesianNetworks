    ...
    if schild==0,
        if isempty(node(id).back),
            if isempty(node(id).value),
                node(id).back = prob_fuse4(node(id).childmsg);
            else
        ...
        node(idx).childmsg(loc,:) = prob_backward4(node(id).p,node(id).parentmsg,card(node(id).parents),sendid,node(id).back);
        ...
        if isempty(node(id).forward),
            if isempty(node(id).value),
                node(id).forward = prob_forward4(node(id).p,node(id).parentmsg,card(node(id).parents));
            else
                if node(id).value>=card(id),
                    error(sprintf('Value of Node #%d is too large!',id));
                end
        ...
        node(idx).parentmsg(loc,1:card(id)) = prob_fuse4([node(id).forward; node(id).childmsg([1:sendid-1 sendid+1:end],:)]);
...
for id=1:N,
    ...
    node(id).pe = prob_fuse4n([node(id).forward; node(id).back]);
end