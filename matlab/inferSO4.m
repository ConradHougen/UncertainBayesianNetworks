...
while ~isempty(active),
    ...
    if schild==0,
        if isempty(node(id).back),
            if isempty(node(id).value),
                if isempty(node(id).childmsg),
                    ...
                    [node(id).back, node(id).backc] = fusion_so4(node(id).childmsg,node(id).childmsgc);
                end
            else
        ...
        [node(idx).childmsg{loc},node(idx).childmsgc{loc}] = backprop_so4(node(id).w,node(id).parentmsg,node(id).parentmsgc,card(node(id).parents),sendid,node(id).back,node(id).backc);
        ...
        if isempty(node(id).forward),
            if isempty(node(id).value),
                if isempty(node(id).parentmsg),
                    ...
                    [node(id).forward,node(id).forwardc] = forwardprop_so4(node(id).w,node(id).parentmsg,node(id).parentmsgc);
                end
            else
        ...
        [node(idx).parentmsg{loc},node(idx).parentmsgc{loc}]= fusion_so4(m_in,cov_in);    
...
for id=1:N,
    ...
    [node(id).me,node(id).mec] = fusion_so4n(m_in,cov_in);
end